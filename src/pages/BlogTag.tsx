...
const BlogTag: React.FC = () => {
    ...
    useEffect(() => {
        if(!BlogSystem.get().hasTag(tagNameDecoded)) {
            window.location.href = "/tags";
            return;
        }

        setArticleList(BlogSystem.get().getBlogsByTag(tagNameDecoded));
    }, []);

    return (
        <Page title={"#"+ tagNameDecoded} singleTitle className={"pt-40 "+ styles["page-content"]}>
            <Section title={"#"+ tagNameDecoded} titleCenterAligned={false} className="mt-6 pl-[20vw] pr-[20vw] max-lg:pl-[5vw] max-lg:pr-[5vw] max-sm:pl-[40px] max-sm:pr-[40px]">
                <div className="mt-8 mb-6 flex justify-between">
                    <span className="text-[--nocp-light-gray]">共 {BlogSystem.get().getTag(tagNameDecoded)?.amount} 篇文章</span>
